...
public class BuildingController : MonoBehaviour {
	...
	private void selectRandomParts(int nbrParts) {
		...
		for (int i = 0; i < nbrParts; i++) {
			...
			this.parts[i].transform.localScale = new Vector3(
				this.parts[i].transform.localScale.x * (Random.Range(0, 2) == 0 ? -1 : 1),
				this.parts[i].transform.localScale.y,
				this.parts[i].transform.localScale.z
			);
	...
	private void createRoof() {
		...
		this.roof.transform.localScale = new Vector3(
			this.roof.transform.localScale.x * (Random.Range(0, 2) == 0 ? -1 : 1),
			this.roof.transform.localScale.y,
			this.roof.transform.localScale.z
		);
	...
	private void createSideLeft(GameObject floor, int sideId) {
		...
		sideWidth = this.buildingParts [sideId].GetComponent<SpriteRenderer> ().sprite.bounds.size.x * this.buildingParts [sideId].transform.localScale.x;
		floorWidth = floor.GetComponent<SpriteRenderer> ().sprite.bounds.size.x * (floor.transform.localScale.x < 0.0f ? -floor.transform.localScale.x : floor.transform.localScale.x);
